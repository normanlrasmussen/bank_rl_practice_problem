...
class BankEnvironment:
    ...
    def _calculate_reward(self, done):
        ...
        if self.reward_type == "sparse":
            # Reward only at end of game: +1 if win, 0 otherwise
            if done:
                if self.opponent is None:
                    return 1.0  # Single player always "wins"
                else:
                    opponent_score = self.bank.player_scores[1]
            ...
            # Score difference (agent_score - opponent_score) at end of game
            if done:
                if self.opponent is None:
            ...
            # Agent's final score (given at end of game)
            if done:
                return float(agent_score)
            else:
                return 0.0
            ...
            # Combination of sparse win bonus + relative score
            if done:
                if self.opponent is None:
                    return 1.0 + float(agent_score)
                else:
                    opponent_score = self.bank.player_scores[1]
                    win_bonus = 1.0 if agent_score > opponent_score else 0.0
                    relative_score = float(agent_score - opponent_score)
                    return win_bonus + relative_score
            else:
                return 0.0
